...
def cluster_split(
    ...
    if balanced:  # Put stuff that's bigger than half the val/test size into train, rest just order randomly
        ...
        index_sets = sorted(list(cluster_to_indices.values()), key=lambda index_set: len(index_set), reverse=True)
    ...
    if logger is not None:
        logger.debug(
            f"Total clusters = {len(cluster_to_indices):,} | "
...
def log_cluster_stats(
    ...
    if logger is not None:
        logger.debug(
            "Label averages per cluster, in decreasing order of cluster frequency,"
