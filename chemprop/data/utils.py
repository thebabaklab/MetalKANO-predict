...
from ..features.utils import load_features
from .data import MoleculeDatapoint, MoleculeDataset
from .scaffold import cluster_split, log_scaffold_stats, scaffold_split
...
def filter_invalid_smiles(data: MoleculeDataset) -> MoleculeDataset:
    ...
    return MoleculeDataset(
        [
            ...
            if datapoint.smiles != "" and datapoint.mol is not None and datapoint.mol.GetNumHeavyAtoms() > 0
        ]
    )
...
def get_data_from_smiles(smiles: List[str], skip_invalid_smiles: bool = True, logger: Logger = None) -> MoleculeDataset:
    ...
    data = MoleculeDataset([MoleculeDatapoint([smile]) for smile in tqdm(smiles, total=len(smiles))])
...
def split_data(
    ...
    if split_type == "crossval":
        ...
        return cluster_split(data, sizes=sizes, balanced=True, seed=seed, logger=logger)
