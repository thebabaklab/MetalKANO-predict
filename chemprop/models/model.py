...
class Prompt_generator(nn.Module):
    ...
    def forward(self, atom_hiddens: torch.Tensor, fg_states: torch.Tensor, atom_num, fg_indexs):
        # Convert fg_indexs to tensor if it's a list of lists
        if isinstance(fg_indexs, list):
            fg_indexs = torch.LongTensor(fg_indexs)
            if atom_hiddens.is_cuda:
                fg_indexs = fg_indexs.cuda()
        
        # Get device from atom_hiddens
        ...
        fg_out = torch.zeros(1, self.hidden_size, device=device)
        cls_hiddens = torch.gather(hidden_states, 0, fg_indexs)
        cls_hiddens = self.linear(cls_hiddens)
        fg_hiddens = torch.repeat_interleave(cls_hiddens, torch.tensor(atom_num, device=device), dim=0)
