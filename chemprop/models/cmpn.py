...
def scatter_add(src, index, dim=0):
    ...
    expand_index = index.unsqueeze(-1).expand_as(src) if src.dim() > 1 else index
    return out.scatter_add_(dim, expand_index, src)

from ..features.featurization import get_atom_fdim, get_bond_fdim, mol2graph
...
class CMPNEncoder(nn.Module):
    ...
    def forward(self, step, mol_graph, features_batch=None) -> torch.FloatTensor:
        ...
        if self.args.step == "functional_prompt":
            ...
            fg_hiddens_atom = torch.repeat_interleave(fg_hiddens, torch.tensor(atom_num, device=device), dim=0)
            fg_out = torch.zeros(1, 133, device=device)
...
class BatchGRU(nn.Module):
    ...
    def forward(self, node, a_scope):
        ...
        message = torch.cat(
            [torch.cat([message.narrow(0, 0, 1), message.narrow(0, 0, 1)], 1), cur_message_unpadding], 0
        )
        return message
...
class CMPN(nn.Module):
    def __init__(self, args: Namespace, atom_fdim: int = None, bond_fdim: int = None, graph_input: bool = False):
        ...
        self.bond_fdim = bond_fdim or get_bond_fdim(args) + (not args.atom_messages) * self.atom_fdim  # * 2
