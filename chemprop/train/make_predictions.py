...
from ..models.model import build_pretrain_model
from ..utils import load_args, load_checkpoint, load_scalers
from .predict import get_emb, predict
...
def make_predictions(args: Namespace, smiles: List[str] = None) -> List[Optional[List[float]]]:
    ...
    if smiles is not None:
        ...
        test_data = get_data(
            path=args.test_path, args=args, use_compound_names=args.use_compound_names, skip_invalid_smiles=False
        )
...
def get_embs(args: Namespace, smiles: List[str] = None) -> List[Optional[List[float]]]:
    ...
    if args.checkpoint_path is not None:
        ...
        for checkpoint_path in tqdm(args.checkpoint_paths, total=len(args.checkpoint_paths)):
            ...
            model_preds = get_emb(model=model, prompt=False, data=test_data, batch_size=args.batch_size, scaler=None)
    else:
        model = build_pretrain_model(args, encoder_name=args.encoder_name)
        model = model.cuda()
