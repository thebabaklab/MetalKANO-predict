...
from ..data.data import MoleculeDataset, StandardScaler
from ..data.utils import get_class_sizes, get_data, get_task_names, split_data
from ..models.loss.loss import ContrastiveLoss
from ..models.model import add_functional_prompt, build_model, build_pretrain_model
from ..nn_utils import param_count
from ..torchlight.logger import initialize_exp
from ..torchlight.utils import snapshot
from ..utils import (
    build_lr_scheduler,
    build_optimizer,
    get_loss_func,
...
def run_training(args: Namespace, prompt: bool, logger: Logger = None) -> List[float]:
    ...
    if args.separate_test_path:
        test_data = get_data(
            path=args.separate_test_path, args=args, features_path=args.separate_test_features_path, logger=logger
        )
    if args.separate_val_path:
        val_data = get_data(
            path=args.separate_val_path, args=args, features_path=args.separate_val_features_path, logger=logger
        )
    ...
    if args.separate_val_path and args.separate_test_path:
        ...
        train_data, _, test_data = split_data(
            data=data, split_type=args.split_type, sizes=(0.8, 0.2, 0.0), seed=args.seed, args=args, logger=logger
        )
    ...
    for model_idx in range(args.ensemble_size):
        ...
        for epoch in range(args.epochs):
            ...
            test_preds = predict(model=model, prompt=prompt, data=test_data, batch_size=args.batch_size, scaler=scaler)
...
def pre_training(args: Namespace, logger: Logger = None) -> List[float]:
    ...
    for model_idx in range(args.ensemble_size):
        ...
        loader = DataLoader(smiles, batch_size=args.batch_size, shuffle=True, num_workers=12, drop_last=True)
        # Run training
        for epoch in range(args.epochs):
