...
from .data.data import StandardScaler
from .models.model import MoleculeModel, add_functional_prompt, build_model
...
def save_checkpoint(
    ...
    state = {
        ...
        "features_scaler": (
            {"means": features_scaler.means, "stds": features_scaler.stds} if features_scaler is not None else None
        ),
    }
...
def load_checkpoint(
    path: str, current_args: Namespace = None, cuda: bool = None, logger: logging.Logger = None
) -> MoleculeModel:
    """
...
def load_scalers(path: str) -> Tuple[StandardScaler, StandardScaler]:
    ...
    features_scaler = (
        StandardScaler(state["features_scaler"]["means"], state["features_scaler"]["stds"], replace_nan_token=0)
...
def build_optimizer(model: nn.Module, args: Namespace) -> Optimizer:
    ...
    params = [
        {"params": no_increase_param, "lr": args.init_lr, "weight_decay": 0},
        {"params": increase_param, "lr": args.init_lr * 5, "weight_decay": 0},
    ]
