...
def normalize(s):
    """
    German and French have different vowels than English.
...
def get_code_version(short_sha=True):
    ...
    try:
        sha = check_output("git rev-parse HEAD", stderr=STDOUT, shell=True, encoding="utf-8")
        if short_sha:
            sha = sha[:7]
        return sha
...
def cat_ragged_tensors(left, right):
    ...
    for i, row_left, row_right, l1, l2 in zip(range(batch_size), left_seq, right_seq, len_left, len_right):
        l1 = l1.item()
        l2 = l2.item()
        j = l1 + l2
...
def topk_accuracy(inputs, labels, k=1, largest=True):
    ...
    result = indices - labels  # broadcast
