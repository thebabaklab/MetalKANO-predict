...
from .features.features_generators import get_available_features_generators
...
def add_predict_args(parser: ArgumentParser):
    ...
    parser.add_argument(
        "--preds_path", type=str, help="Path to CSV file where predictions will be saved", default="test_pred"
    )
    parser.add_argument(
        "--checkpoint_dir",
        type=str,
    ...
    parser.add_argument(
        "--features_path", type=str, nargs="*", help="Path to features to use in FNN (instead of features_generator)"
    )
    parser.add_argument(
        "--no_features_scaling", action="store_true", default=False, help="Turn off scaling of features"
    )
...
def add_train_args(parser: ArgumentParser):
    ...
    parser.add_argument("--max_data_size", type=int, help="Maximum number of data points to load")
    parser.add_argument(
        "--test", action="store_true", default=False, help="Whether to skip training and only test the model"
    )
    parser.add_argument(
        "--features_only",
        action="store_true",
    ...
    parser.add_argument(
        "--save_dir", type=str, default="./ckpt", help="Directory where model checkpoints will be saved"
    )
    parser.add_argument(
        "--save_smiles_splits",
        action="store_true",
    ...
    parser.add_argument(
        "--multiclass_num_classes", type=int, default=3, help="Number of classes when running multiclass classification"
    )
    parser.add_argument("--separate_val_path", type=str, help="Path to separate val set, optional")
    parser.add_argument(
        "--separate_val_features_path", type=str, nargs="*", help="Path to file with features for separate val set"
    )
    parser.add_argument("--separate_test_path", type=str, help="Path to separate test set, optional")
    parser.add_argument(
        "--separate_test_features_path", type=str, nargs="*", help="Path to file with features for separate test set"
    )
    parser.add_argument(
        "--split_type",
        type=str,
    ...
    parser.add_argument("--num_runs", type=int, default=1, help="Number of runs when performing k independent runs")
    parser.add_argument("--folds_file", type=str, default=None, help="Optional file of fold labels")
    parser.add_argument(
        "--val_fold_index", type=int, default=None, help="Which fold to use as val for leave-one-out cross val"
    )
    parser.add_argument(
        "--test_fold_index", type=int, default=None, help="Which fold to use as test for leave-one-out cross val"
    )
    parser.add_argument(
        "--crossval_index_dir", type=str, help="Directory in which to find cross validation index files"
    )
    parser.add_argument(
        "--crossval_index_file",
        type=str,
        help="Indices of files to use as train/val/test" "Overrides --num_folds and --seed.",
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    parser.add_argument(
        "--log_frequency", type=int, default=10, help="The number of batches between each logging of the training loss"
    )
    parser.add_argument("--no_cuda", action="store_true", default=False, help="Turn off cuda")
    parser.add_argument(
    ...
    parser.add_argument(
        "--encoder_name", type=str, default="CMPNN", choices=["CMPNN", "MPNN"], help="Name of the encoder"
    )
    ...
    parser.add_argument(
        "--ffn_hidden_size", type=int, default=None, help="Hidden dim for higher-capacity FFN (defaults to hidden_size)"
    )
    parser.add_argument("--ffn_num_layers", type=int, default=2, help="Number of layers in FFN after MPN encoding")
    parser.add_argument(
        "--atom_messages", action="store_true", default=False, help="Use messages on atoms instead of messages on bonds"
    )
    parser.add_argument("--dump_path", default="dumped", type=str, help="Experiment dump path")
    parser.add_argument("--exp_name", default="", type=str, required=True, help="Experiment name")
    parser.add_argument("--exp_id", default="", type=str, required=True, help="Experiment ID")
    parser.add_argument(
        "--step",
        default="functional_prompt",
        type=str,
...
def modify_train_args(args: Namespace):
    ...
    if not (
        (args.dataset_type == "classification" and args.metric in ["auc", "prc-auc", "accuracy"])
        or (args.dataset_type == "regression" and args.metric in ["rmse", "mae", "mse", "r2"])
        or (args.dataset_type == "multiclass" and args.metric in ["cross_entropy", "accuracy"])
    ):
