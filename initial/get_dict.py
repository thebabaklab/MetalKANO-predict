...
property2emb = {}
for p in range(len(objectproperty)):
    p_name = URIRef("http://www.semanticweb.org/ElementKG#" + objectproperty[p])
    pro_emb = pro2emb[p]
...
for i in range(len(element_symbols) - 1):
    for j in range(1, len(element_symbols)):
        qr = (
            "select ?relation where { <http://www.semanticweb.org/ElementKG#"
            + element_symbols[i]
            + "> ?relation <http://www.semanticweb.org/ElementKG#"
            + element_symbols[j]
            + ">}"
        )
